...
def train_genesis(
    ...
    if random_name is None:
        random_name = get_random_name("CL")
    ...
    # wandb 초기화
    run = wandb.init(
        project="(2)curriculum_learning",
        name=f"{REPLAY_RATIO}_{random_name}_{gamma}_{learning_rate}_{END_SUCCESS_RATE}",
        config=config,
        monitor_gym=True,
        save_code=True,
    )
    ...
    # 실험 결과 저장 디렉토리 생성
    save_path = os.path.join("training_results/Ex(2)", random_name)
    ...
    try:
        ...
        # 최종 모델 저장
        final_model_path = os.path.join(save_path, f"{REPLAY_RATIO}_{random_name}_{gamma}_{learning_rate}_{END_SUCCESS_RATE}_final.zip")
        model.save(final_model_path)
        
        # 최종 모델을 W&B에 업로드
        artifact = wandb.Artifact(name=f"{REPLAY_RATIO}_{random_name}_{gamma}_{learning_rate}_{END_SUCCESS_RATE}_final", type="model")
